...
def aggregate_public_archives(folder_path):
    ...
    for file in file_names:
        try:
            df = pd.read_excel(folder_path + file, sheet_name="Archive publique", engine='calamine')
            df_list.append(df)
        except:
...
def get_all_project_names(path):
    ...
    for i, projet in enumerate(all_project_names):    #remove "_Terminé - "
        if "_Terminé - " in projet: all_project_names[i] = ''.join(projet.split("_Terminé - "))
