...
class suivi_projet_GUI(tk.Tk):
    ...
    def init_window_2(self):
        ...
        def agreger():
            ...
            used_project_names = self.aggregated_public_archives["Projet ?"].unique()
            
            for i, projet in enumerate(used_project_names):
                if "_Terminé - " in projet: used_project_names[i] = ''.join(projet.split("_Terminé - "))
            ...
            if len(suspect_project_names)>0:
                ...
                for suspect_project in suspect_project_names:
                    ...
                    warning_message += " - "+ suspect_project +": "+ ', '.join(people)+"\n"
        ...
        def update_done_vs_todo(*args):
            ...
            if len(nan_pos)>0:
                message = '\n - ' + '\n - '.join([f"" for i in nan_pos])
                for i in nan_pos:
                    next_to = []
                    if i>0: next_to.append(df["Qui ?"].iloc[i-1])
                    if i<len(df): next_to.append(df["Qui ?"].iloc[i+1])
                    message += f"\n - next to {np.unique(next_to)}"
                        
                tk.messagebox.showwarning("",f"Warning : Identités manquantes (x{len(nan_pos)}) lors de la lecture des données agrégées."+message)
