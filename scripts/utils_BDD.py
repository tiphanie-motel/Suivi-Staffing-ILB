...
def load_current_data(path_to_folder, add_old_aggregated_data=False):
    ...
    if not mask_projet.all(): 
        log += "ATTENTION : Problème lors du chargement des données agrégées. "+str(np.sum(~mask_projet))+" lignes avec nom de projet erroné.\n"
        print(aggregated_data[~mask_projet])
    if not mask_personne.all(): 
        log += "ATTENTION : Problème lors du chargement des données agrégées. "+str(np.sum(~mask_personne))+" lignes avec nom de personne erroné.\n"
        print(aggregated_data[~mask_personne])
        ...
        if prenom_nom in prenom_nom_removed: 
            ...
            if prenom_nom in prenom_nom_added: 
                ...
                if len(appartenance_lab_prev)!=len(appartenance_lab_new) or appartenance_lab_prev!=appartenance_lab_new:
                    changes[2] = True
                    data_to_append[3] += [threshold_date, [labs_new, [float(e) for e in appartenance_lab_new]]]
